...
(function($B) {
    ...
    $B.__setattr__ = function(attr, value) {
        if (['debug'].indexOf(attr) > -1) {
            $B[attr] = value
        } else {
            throw $B.builtins.AttributeError('__BRYTHON__ object has no attribute ' + attr)
        }
    }
    ...
    if ($B.has_local_storage) {
        $B.local_storage = function() {
            if (typeof localStorage.getItem === 'function') {
                ...
                res.__item__ = function(rank) {
                    return localStorage.key(rank)
                }
                return res
            }
            var res = new Object()
            ...
            res.getItem = function(key) {
                return localStorage.getItem(str(key))
            }
            res.setItem = function(key, value) {
                localStorage.setItem(str(key), str(value))
            }
            return res
        }
    }
    ...
    if ($B.has_indexedDB) {
        $B.indexedDB = function() {
            return $B.JSObject($B._indexedDB)
        }
    }
    $B.re = function(pattern, flags) {
        return $B.JSObject(new RegExp(pattern, flags))
    }
    $B.has_json = typeof(JSON) !== "undefined"
    $B.has_websocket = (function() {
        try {
            var x = window.WebSocket;
            return x !== undefined
        } catch (err) {
            return false
        }
...
(function($B) {
    ...
    function $_SyntaxError(C, msg, indent) {
        ...
        if (indent === undefined) {
            if (msg.constructor === Array) {
                $B.$SyntaxError(module, msg[0], $pos)
            }
            ...
            switch (left.type) {
                case 'expr':
                    if (left.tree.length > 1) {
                        ...
                        if (scope.bound.indexOf(name) == -1) {
                            scope.bound.push(name)
                        }
                    }
                    break
                case 'target_list':
    ...
    function $AugmentedAssignCtx(C, op) {
        ...
        this.transform = function(node, rank) {
            ...
            if (prefix) {
                var new_node = new $Node()
                js = right_is_int ? 'if(' : 'if(typeof $temp=="number" && '
        ...
        this.to_js = function() {
            return ''
            if (this.tree[0].type == 'expr' && this.tree[0].length == 1 && this.tree[0].tree[0].type == 'id') {
                ...
                if (this.tree.length > 0) {
                    ...
                    if (this.func.type == 'id') {
                        ...
                        for (var i = 0; i < this.tree.length; i++) {
                            if (this.tree[i].type == 'id') {
                                continue
                            }
                            if (this.tree[i].tree[0].type != 'expr') {
                                $simple = false;
                                break
                            }
                        }
    ...
    function $ClassCtx(C) {
        ...
        this.transform = function(node, rank) {
            ...
            var js = 'var $class={}'
            if ($B.debug > 0) {
                js = 'var $class = {$def_line:__BRYTHON__.line_info}'
            }
            ...
            scope.bound = scope.bound || []
            if (scope.bound.indexOf(this.name) == -1) {
                scope.bound.push(this.name)
            }
            this.transformed = true
        }
        this.to_js = function() {
    ...
    function $DefCtx(C) {
        ...
        this.set_name = function(name) {
            ...
            if (this.scope.bound.indexOf(name) == -1) {
                this.scope.bound.push(name)
            }
            if (scope.is_function) {
        ...
        this.transform = function(node, rank) {
            ...
            for (var i = 0; i < this.tree[0].tree.length; i++) {
                ...
                } else if (arg.type === 'func_star_arg' && arg.op === '*') {
                    other_args = '"' + arg.name + '"'
                } else if (arg.type === 'func_star_arg' && arg.op === '**') {
                    other_kw = '"' + arg.name + '"'
                }
            ...
            if (defaults.length == 0 && other_args === null && other_kw === null &&
                ...
                for (var i = 0; i < required_list.length; i++) {
                    ...
                    var new_node = new $Node()
                    new $NodeJSCtx(new_node, 'var ' + arg + '=$locals["' + arg + '"]=arguments[' + i + ']')
                    else_node.add(new_node)
                }
            } else {
            ...
            if (scope.ntype === "module" || scope.ntype !== 'class') {} else {
                func_ref = '$class.' + func_ref
            }
    ...
    function $FromCtx(C) {
        ...
        this.to_js = function() {
            ...
            if (mod.substr(0, 13) === '__main__,exec') {
                mod = '__main__'
            }
            ...
            for (var i = 0; i < scope.var2node[varname].length; i++) {
                ...
                if (ctx == assigned) {
                    ...
                    for (var rank = 0; rank < pnode.children.length; rank++) {
                        if (pnode.children[rank] === ctx_node) {
                            break
                        }
                    }
                    var new_node = new $Node()
            ...
            switch (val) {
                ...
                case 'globals':
                    if (this.parent.type === 'call') {
                        var scope = $get_scope(this)
                        if (scope.ntype === "module") {
                            new $StringCtx(this.parent, '"__main__"')
                        } else {
    ...
    function $NonlocalCtx(C) {
        ...
        this.add = function(name) {
            if (this.scope.globals.indexOf(name) == -1) {
                this.scope.globals.push(name)
            }
        }
        this.to_js = function() {
            return ''
            ...
            switch (this.op) {
                ...
                case '-':
                    ...
                    function is_simple(elt) {
                        return (elt.type == 'expr' && ['id', 'int', 'float'].indexOf(elt.tree[0].type) > -1)
                    }
                    var e0 = this.tree[0],
                        e1 = this.tree[1]
                    if (is_simple(e0) && is_simple(e1)) {
                        var v0 = this.tree[0].tree[0]
                        var v1 = this.tree[1].tree[0]
                        if (v0.type == 'int' && v1.type == 'int') {
                            return v0.value + op + v1.value
                        } else if (v0.type != 'id' && v1.type != 'id') {
                            return 'float(' + v0.value + op + v1.value + ')'
                        } else if (v0.type != 'id') {
                            var res = '(typeof ' + v1.value + '.valueOf()=="number") ? '
                            if (v0.type == 'int') {
                                res += '(typeof ' + v1.value + '=="number" ? '
                                res += v0.value + op + v1.value + ' : '
                                res += 'float(' + v0.value + op + v1.value + '.value)) : '
                            } else {
                                res += 'float(' + v0.value + op + v1.value + ') : '
                            }
                        } else if (v1.type != 'id') {
                            var res = '(typeof ' + v0.value + '.valueOf()=="number") ? '
                            if (v1.type == 'int') {
                                res += '(typeof ' + v0.value + '=="number" ? '
                                res += v0.value + op + v1.value + ' : '
                                res += 'float(' + v1.value + op + v0.value + '.value)) : '
                            } else {
                                res += 'float(' + v0.value + op + v1.value + ') : '
                            }
                        } else {
                            if (v0.value == v1.value) {
                                ...
                                var res = '(typeof ' + v0.value + '.valueOf()=="number" &&'
                                res += ' typeof ' + v1.value + '.valueOf()=="number") ? '
                                res += '(typeof ' + v0.value + '=="number" '
                                res += '&& typeof ' + v1.value + '=="number" ?'
                                res += v0.value + op + v1.value + ' : '
                                res += 'float(' + v0.value + op + v1.value + ')) :'
        ...
        this.transform = function(node, rank) {
            ...
            while (1) {
                ...
                if (ctx.type === 'except') {
                    ...
                    if (has_else) {
                        $_SyntaxError(C, "'finally' after 'else'")
                    }
                    pos++
    ...
    function $add_line_num(node, rank) {
        if (node.type === 'module') {
            ...
            if (node.module === undefined) {
                ...
                if (node.module === undefined) {
                    console.log('module undef, node ' + node.C);
                    flag = false
                }
            }
        ...
        switch (C.type) {
                ...
                switch (token) {
                    ...
                    case 'op':
                        ...
                        while (1) {
                            if (op1.type === 'expr') {
                                op1 = op1.parent
                            } else if (op1.type === 'op' && $op_weight[op1.op] >= $op_weight[op]) {
                                repl = op1;
                                op1 = op1.parent
                            } else {
                                break
            ...
            case 'from':
                switch (token) {
                    ...
                    case 'op':
                        if (arguments[2] === '*' && C.expect === 'id' && C.names.length === 0) {
                            C.names.push('*')
                            C.expect = 'eol'
                            return C
                        }
            ...
            case 'with':
                switch (token) {
                    case 'id':
                        ...
                        break
                    case 'as':
                        if (C.expect === 'as' && C.has_alias === undefined && C.tree.length === 1) {
                            C.expect = 'alias'
                            C.has_alias = true
                            return C
                        }
        ...
        while (pos < src.length) {
            ...
            if (car == ".") {
                if (pos < src.length - 1 && /^\d$/.test(src.charAt(pos + 1))) {
                    var j = pos + 1
                    while (j < src.length && src.charAt(j).search(/\d/) > -1) {
                        j++
                    }
...
__BRYTHON__.$__new__ = function(factory) {
    return function(cls) {
        ...
        var init_func = null
        try {
            init_func = __BRYTHON__.builtins.getattr(res, '__init__')
        } catch (err) {
            __BRYTHON__.$pop_exc()
        }
        if (init_func !== null) {
...
__BRYTHON__.builtins.object = (function($B) {
    ...
    $ObjectDict.__getattribute__ = function(obj, attr) {
        ...
        if (res !== undefined) {
            ...
            if (get_func === undefined && (typeof res == 'object')) {
                ...
                if (__get__ && (typeof __get__ == 'function')) {
                    get_func = function(x, y) {
                        return __get__.apply(x, [y, klass])
                    }
                }
            }
            ...
            if (_ga === undefined) {
                var mro = klass.__mro__
                if (mro === undefined) {
                    console.log('in getattr mro undefined for ' + obj)
                }
                for (var i = 0; i < mro.length; i++) {
    ...
    $ObjectDict.__new__ = function(cls) {
        if (cls === undefined) {
            throw _b_.TypeError('object.__new__(): not enough arguments')
        }
        var obj = new Object()
    ...
    $ObjectDict.__setattr__.__str__ = function() {
    ...
    function object() {
        return {
            __class__: $ObjectDict
        }
    }
    object.$dict = $ObjectDict
    $ObjectDict.$factory = object
    object.__repr__ = object.__str__ = function() {
        return "<class 'object'>"
    }
    return object
})(__BRYTHON__);
(function($B) {
    ...
    _b_.type = function(name, bases, cl_dict) {
        if (arguments.length == 1) {
            return $B.get_class(name).$factory
        }
        ...
        for (var i = 0; i < cl_dict.$keys.length; i++) {
            var attr = cl_dict.$keys[i],
                val = cl_dict.$values[i]
            class_dict[attr] = val
        }
        var seqs = []
    ...
    $B.$type.__getattribute__ = function(klass, attr) {
        ...
        if (res === undefined) {
            var mro = klass.__mro__
            if (mro === undefined) {
                console.log('mro undefined for class ' + klass + ' name ' + klass.__name__)
            }
            for (var i = 0; i < mro.length; i++) {
                var v = mro[i][attr]
                if (v !== undefined) {
    ...
    function $instance_creator(klass) {
        return function() {
            ...
            if (!obj.__initialized__) {
                try {
                    init_func = _b_.getattr(klass, '__init__')
                } catch (err) {
                    $B.$pop_exc()
                }
                if (init_func !== null) {
    ...
    $MethodFactory.__repr__ = $MethodFactory.__str__ = function() {
        return 'method'
    }
    $B.$MethodDict = {
        __class__: $B.$type,
...
(function($B) {
    ...
    $B.$MakeArgs = function($fname, $args, $required, $defaults, $other_args, $other_kw, $after_star) {
        ...
        for (var i = 0; i < $args.length; i++) {
            $arg = $args[i]
            if ($arg === undefined) {
                console.log('arg ' + i + ' undef in ' + $fname)
            } else if ($arg === null) {
                upargs.push(null)
            } else {
                switch ($arg.$nat) {
        ...
        for (var i = 0; i < $required.length; i++) {
            if ($set_vars.indexOf($required[i]) == -1) {
        ...
        if ($other_args != null) {
            $ns[$other_args] = _b_.tuple($ns[$other_args])
        }
        return $ns
    }
    ...
    $B.$gen_expr = function() {
        ...
        $GenExprDict.__iter__ = function(self) {
            return self
        }
        ...
        var $res2 = {
            value: $res1,
            __class__: $GenExprDict,
            $counter: -1
        }
        $res2.toString = function() {
            return 'ge object'
        }
        return $res2
    }
...
(function($B) {
    ...
    $B.make_node = function(top_node, node) {
        ...
        if (node.C.type == 'node') {
            ...
            if (ctype == 'condition' && ctx.token == 'elif') {
                is_else = true
            }
            if (ctype == 'single_kw' && ctx.token == 'else') {
                is_else = true
            }
            if (ctype == 'except' ||
                (ctype == 'single_kw' && ctx.token == "finally")) {
                is_except = true
            }
        }
        if (ctx_js) {
    ...
    $BRGeneratorDict.__iter__ = function(self) {
        return self
    }
    $BRGeneratorDict.__enter__ = function(self) {
        console.log("generator.__enter__ called")
    }
    $BRGeneratorDict.__exit__ = function(self) {
    ...
    $BRGeneratorDict.__next__ = function(self) {
        ...
        if (res[0].__class__ == $GeneratorReturn) {
            self._next = function() {
                throw StopIteration("after generator return")
            }
            throw StopIteration('')
        }
        var yielded_value = res[0],
            yield_rank = res[1]
        ...
        while (prest !== func_node) {
            if (prest.is_except) {
                ...
                if (prest.parent.is_except) {
                    catch_node = prest.parent
                }
                var rank = catch_node.rank
                while (rank < catch_node.parent.children.length &&
                    catch_node.parent.children[rank].is_except) {
                    rank++
                }
        ...
        while (pnode !== func_node && in_loop(pnode)) {
            var rank = pnode.rank
            while (pnode.parent.children[rank].is_except) {
                rank--
            }
            while (pnode.parent.children[rank].is_else) {
                rank--
            }
    ...
    $B.$BRgenerator.__repr__ = function() {
        return "<class 'generator'>"
    }
    $B.$BRgenerator.__str__ = function() {
        return "<class 'generator'>"
    }
...
(function($B) {
    ...
    bool.__name__ = 'bool'
    bool.__repr__ = bool.__str__ = function() {
        return "<class 'bool'>"
    }
    ...
    $FilterDict.__repr__ = $FilterDict.__str__ = function() {
    ...
    function getattr(obj, attr, _default) {
        ...
        var is_class = klass.is_class,
            mro, attr_func
        if (is_class) {
            attr_func = $B.$type.__getattribute__
            if (obj.$dict === undefined) {
                console.log('obj ' + obj + ' $dict undefined')
            }
            obj = obj.$dict
        } else {
            var mro = klass.__mro__
    ...
    function isinstance(obj, arg) {
        ...
        if (klass !== undefined) {
            if (klass.__mro__ === undefined) {
                console.log('mro undef for ' + klass + ' ' + klass.__name___ + ' ' + dir(klass) + '\n arg ' + arg)
            }
            if (arg.$dict === undefined) {
                return false
            }
    ...
    $MapDict.__iter__ = function(self) {
        return self
    }
    ...
    function map() {
        ...
        var iter_args = []
        for (var i = 1; i < arguments.length; i++) {
            iter_args.push(iter(arguments[i]))
        }
        var __next__ = function() {
            var args = []
    ...
    function $extreme(args, op) {
        ...
        if (last_arg.$nat == 'kw') {
            if (last_arg.name === 'key') {
                ...
                throw _b_.TypeError("'" + last_arg.name + "' is an invalid keyword argument for this function")
            }
        } else {
            var func = function(x) {
                return x
        ...
        if (second_to_last_arg && second_to_last_arg.$nat == 'kw') {
            if (second_to_last_arg.name === 'key') {
                if (has_key) {
                    throw _b_.SyntaxError("Keyword argument repeated")
                }
                ...
                throw _b_.TypeError("'" + second_to_last_arg.name + "' is an invalid keyword argument for this function")
            }
        } else {
            if (!func) {
                var func = function(x) {
        ...
        if ((has_key && has_default && args.length == 3) ||
            ...
            while (true) {
                try {
                    var x = next($iter)
                    if (res === null || bool(getattr(func(x), op)(func(res)))) {
                        res = x
                    }
                } catch (err) {
                    if (err.__name__ == "StopIteration") {
    ...
    $NotImplementedDict.__repr__ = $NotImplementedDict.__str__ = function() {
    ...
    function pow() {
        ...
        if (args.length === 2) {
            ...
            var y = args[1]
            var a, b
            if (isinstance(x, _b_.float)) {
                a = x.value
            } else if (isinstance(x, _b_.int)) {
                a = x
            } else {
                throw _b_.TypeError("unsupported operand type(s) for ** or pow()")
            }
            if (isinstance(y, _b_.float)) {
                b = y.value
            } else if (isinstance(y, _b_.int)) {
                b = y
            } else {
                throw _b_.TypeError("unsupported operand type(s) for ** or pow()")
            }
    ...
    $ReversedDict.__iter__ = function(self) {
        return self
    }
    ...
    function reversed(seq) {
        try {
            ...
            if (err.__name__ == 'AttributeError') {
                $B.$pop_exc()
            } else {
                throw err
            }
    ...
    function sorted() {
        ...
        var args = [obj]
        if (key !== None) args.push({
            $nat: 'kw',
            name: 'key',
            value: key
        })
        if (reverse) args.push({
            $nat: 'kw',
            name: 'reverse',
            value: true
        })
    ...
    var $StaticmethodDict = {
        __class__: $B.$type,
        __name__: 'staticmethod'
    }
    ...
    $BoolDict.__le__ = function(self, other) {
        return !$BoolDict.__gt__(self, other)
    }
    $BoolDict.__lt__ = function(self, other) {
        return !$BoolDict.__ge__(self, other)
    }
    ...
    var $FunctionDict = $B.$FunctionDict = {
        __class__: $B.$type,
        __code__: {
            __class__: $FunctionCodeDict,
            __name__: 'function code'
        },
        __globals__: {
            __class__: $FunctionGlobalsDict,
            __name__: 'function globals'
        },
        __name__: 'function'
    }
    ...
    $BaseExceptionDict.__repr__ = function(self) {
        if (self.message === None) {
            return $B.get_class(self).__name__ + '()'
        }
        return self.message
    }
...
(function($B) {
    ...
    $BytesDict.__len__ = function(self) {
    ...
    $BytesDict.__ne__ = function(self, other) {
        return !$BytesDict.__eq__(self, other)
    }
    ...
    $BytesDict.__reduce_ex__ = function(self) {
        return $BytesDict.__repr__(self)
    }
    ...
    $BytesDict.lstrip = function(self, cars) {
        return _strip(self, cars, 'l')
    }
    $BytesDict.rstrip = function(self, cars) {
    ...
    for (var $attr in $BytesDict) {
        if ($BytearrayDict[$attr] === undefined) {
            $BytearrayDict[$attr] = (function(attr) {
                return function() {
                    return $BytesDict[attr].apply(null, arguments)
                }
            })($attr)
        }
    }
...
(function($B) {
    ...
    $JSObjectDict.__getattribute__ = function(obj, attr) {
        ...
        if (attr == "bind" && obj.js[attr] === undefined &&
            obj.js['addEventListener'] !== undefined) {
    ...
    $JSObjectDict.__repr__ = function(self) {
        return "<JSObject wraps " + self.js.toString() + ">"
    }
...
(function($B) {
    ...
    $B.$ModuleDict.__repr__ = function(self) {
        return '<module ' + self.__name__ + '>'
    }
    $B.$ModuleDict.__str__ = function(self) {
        return '<module ' + self.__name__ + '>'
    }
    ...
    function $importer() {
        ...
        } else if (window.XMLHttpRequest) {} else {
        ...
        var timer = setTimeout(function() {
            $xmlhttp.abort()
            throw _b_.ImportError("No module named '" + module + "'")
        }, 5000)
    ...
    function import_from_caller_folder(mod_name, origin, package) {
        ...
        while (mod_elts[0] == origin_elts[0]) {
            mod_elts.shift();
            origin_elts.shift()
        }
    ...
    $B.$import = function(mod_name, origin) {
        if ($B.imported[origin] === undefined) {
            var package = ''
        } else {
            var package = $B.imported[origin].__package__
        }
        if ($B.$options.debug == 10) {
    ...
    $B.$import_from = function(mod_name, names, origin) {
        if ($B.$options.debug == 10) {
            console.log('import from ' + mod_name);
            show_ns()
        }
        if (mod_name.substr(0, 2) == '$$') {
            mod_name = mod_name.substr(2)
...
(function($B) {
    var _b_ = $B.builtins
    for (var $py_builtin in _b_) {
        eval("var " + $py_builtin + "=_b_[$py_builtin]")
    }
    var $ObjectDict = _b_.object.$dict
    ...
    $FloatDict.__bool__ = function(self) {
        return _b_.bool(self.value)
    }
    ...
    $FloatDict.is_integer = function(self) {
        return _b_.int(self.value) == self.value
    }
    ...
    $FloatDict.__ne__ = function(self, other) {
        return !$FloatDict.__eq__(self, other)
    }
    $FloatDict.__neg__ = function(self, other) {
...
(function($B) {
    ...
    $IntDict.__bool__ = function(self) {
    ...
    $IntDict.__index__ = function(self) {
        return self
    }
    ...
    $IntDict.__int__ = function(self) {
        return self
    }
    $IntDict.__invert__ = function(self) {
        return ~self
    }
    ...
    $IntDict.__ne__ = function(self, other) {
        return !$IntDict.__eq__(self, other)
    }
    $IntDict.__neg__ = function(self) {
        return -self
    }
    $IntDict.__new__ = function(cls) {
        if (cls === undefined) {
            throw _b_.TypeError('int.__new__(): not enough arguments')
        }
        return {
            __class__: cls.$dict
        }
    ...
    var int = function() {
        ...
        if (!isinstance(base, _b_.int)) {
            if (hasattr(base, '__int__')) {
                base = Number(getattr(base, '__int__')())
            } else if (hasattr(base, '__index__')) {
                base = Number(getattr(base, '__index__')())
            }
        }
        if (!(base >= 2 && base <= 36)) {
...
(function($B) {
    ...
    $ComplexDict.__abs__ = function(self, other) {
        return complex(abs(self.real), abs(self.imag))
    }
    $ComplexDict.__bool__ = function(self) {
        return new Boolean(self.real || self.imag)
    }
    ...
    $ComplexDict.__invert__ = function(self) {
        return ~self
    }
    ...
    $ComplexDict.__ne__ = function(self, other) {
        return !$ComplexDict.__eq__(self, other)
    }
    $ComplexDict.__neg__ = function(self) {
...
(function($B) {
    ...
    $DictDict.__ne__ = function(self, other) {
        return !$DictDict.__eq__(self, other)
    }
    ...
    $DictDict.get = function(self, key, _default) {
        try {
            return $DictDict.__getitem__(self, key)
        } catch (err) {
            $B.$pop_exc()
            if (_default !== undefined) return _default
            return None
...
(function($B) {
    ...
    $ListDict.__ge__ = function(self, other) {
        ...
        while (i < self.length) {
            ...
            if (getattr(self[i], '__eq__')(other[i])) {
                i++
    ...
    $ListDict.__ne__ = function(self, other) {
    ...
    $ListDict.__repr__ = function(self) {
        ...
        for (var i = 0; i < self.length; i++) {
            var x = self[i]
            try {
                res += getattr(x, '__repr__')()
            } catch (err) {
                console.log('no __repr__');
                res += x.toString()
            }
            if (i < self.length - 1) {
                res += ', '
    ...
    $ListDict.append = function(self, other) {
        self.push(other)
    }
    ...
    $ListDict.insert = function(self, i, item) {
    ...
    $ListDict.sort = function(self) {
        ...
        for (var i = 1; i < arguments.length; i++) {
            ...
            if (arg.$nat == 'kw') {
                if (arg.name === 'key') {
                    func = getattr(arg.value, '__call__')
                } else if (arg.name === 'reverse') {
                    reverse = arg.value
                }
            }
        }
...
(function($B) {
    ...
    $StringDict.__len__ = function(self) {
        return self.length
    }
    var $legacy_format = $StringDict.__mod__ = function(self, args) {
        ...
        function format(s) {
            ...
            this.src = res[0]
            if (res[1]) {
                this.mapping_key = str(res[1].substr(1, res[1].length - 2))
            } else {
                this.mapping_key = null
            }
            this.flag = res[2]
            ...
            this.format = function(src) {
                ...
                switch (this.type) {
                    case 's':
                        var res = str(src)
                        if (this.precision) {
                            return res.substr(0, parseInt(this.precision.substr(1)))
                        }
                        return res
                    case 'r':
                        var res = repr(src)
                    ...
                    case 'G':
                        ...
                        var prec = -4
                        if (this.precision) {
                            prec = parseInt(this.precision.substr(1))
                        }
                    ...
                    case 'F':
                        ...
                        if (this.min_width) {
                            ...
                            while (res.length < parseInt(this.min_width)) {
                                res = pad + res
                            }
                        }
                        return res
    ...
    $StringDict.__ne__ = function(self, other) {
        return other !== self.valueOf()
    }
    ...
    $StringDict.__setattr__ = function(self, attr, value) {
    ...
    $StringDict.istitle = function(self) {
        return /^([A-Z][a-z]+)(\s[A-Z][a-z]+)$/i.test(self)
    }
    ...
    $StringDict.rindex = function() {
        ...
        if (res == -1) {
            throw _b_.ValueError("substring not found")
        }
        return res
    }
    ...
    $StringDict.upper = function(self) {
    ...
    for (var method in $StringDict) {
        if ($B.$AttrDict[method] !== undefined) {
            continue
        }
...
(function($B) {
    ...
    $SetDict.__contains__ = function(self, item) {
        ...
        for (var i = 0; i < self.$items.length; i++) {
            try {
                if (getattr(self.$items[i], '__eq__')(item)) return true
            } catch (err) {
                void(0)
            }
        }
    ...
    $SetDict.__hash__ = function(self) {
        throw _b_.TypeError("unhashable type: 'set'");
    }
    ...
    $SetDict.__ne__ = function(self, other) {
        return !$SetDict.__eq__(self, other)
    }
...
(function($B) {
    ...
    $B.$isNodeList = function(nodes) {
        try {
            ...
            return (typeof nodes === 'object' && re.exec(result) !== null && nodes.hasOwnProperty('length') && (nodes.length == 0 || (typeof nodes[0] === "object" && nodes[0].nodeType > 0)))
        } catch (err) {
            return false
        }
    }
            ...
            return function(attr, value) {
                if (attr.substr(0, 2) == 'on') {
                    if (window.addEventListener) {
                        ...
                        var callback = function(ev) {
                            return value($DOMEvent(window.event))
                        }
    ...
    $OptionsDict.__len__ = function(self) {
    ...
    $OptionsDict.remove = function(self, arg) {
        self.parent.options.remove(arg.elt)
    }
    var $StyleDict = {
        __class__: $B.$type,
        __name__: 'CSSProperty'
    }
    ...
    DOMNode.__add__ = function(self, other) {
        ...
        } else if (isinstance(other, [_b_.str, _b_.int, _b_.float, _b_.list, _b_.dict, _b_.set, _b_.tuple])) {
    ...
    DOMNode.__getitem__ = function(self, key) {
        if (self.elt.nodeType === 9) {
            if (typeof key === "string") {
                ...
                try {
                    var elts = self.elt.getElementsByTagName(key.$dict.__name__),
                        res = []
    ...
    DOMNode.__len__ = function(self) {
        return self.elt.childNodes.length
    }
    ...
    DOMNode.__setattr__ = function(self, attr, value) {
        if (attr.substr(0, 2) == 'on') {
            ...
            if (self.elt[attr1] !== undefined) {
                self.elt[attr1] = value;
                return
            }
    ...
    DOMNode.getContext = function(self) {
        ...
        var obj = self.elt
        return function(ctx) {
            return JSObject(obj.getContext(ctx))
        }
    }
    ...
    DOMNode.remove = function(self, child) {
        ...
        while (ch_elt.parentElement) {
            if (ch_elt.parentElement === elt) {
                ...
                ch_elt = ch_elt.parentElement
            }
        }
        if (!flag) {
            throw _b_.ValueError('element ' + child + ' is not inside ' + self)
        }
    }
    ...
    DOMNode.prototype.css = function(property, value) {
        ...
        if (this.style[property] === undefined) {
            return None
        }
