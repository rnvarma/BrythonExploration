...
function $DefCtx(context){
    ...
    this.transform = function(node,rank){
        ...
        for(var i=0;i<this.env.length;i++){
           var env=this.env[i]
           if(scope.ntype=='class'){
             txt += '$class.'+ env +' != undefined ? $class.' + env + ' : '
           }
           txt += env
...
function $ForExpr(context){
    ...
    this.transform = function(node,rank){
        ...
        if(scope.ntype!='BRgenerator' && scope.ntype!='class'){
            ...
            // node to test if iterator has __getitem__
...
function $OpCtx(context,op){ // context is the left operand
    ...
    this.to_js = function(){
        switch(this.op) {
          ...
          case 'is':
            //this should really be id(....) == id(....)
            //return 'id('+this.tree[0].to_js() + ')== id('+this.tree[1].to_js()+')'
...
function $transition(context,token){
    ...
    switch(context.type) {
      ...
      case 'comprehension':
        ...
        if(token==='for') return new $TargetListCtx(new $CompForCtx(context))
        return $transition(context.parent,token,arguments[2])
      case 'condition':
        if(token===':') return $BodyCtx(context)
        $_SyntaxError(context,'token '+token+' after '+context)
      ...
      case 'def':
        ...
        if(token==='(') {context.has_args=true;return new $FuncArgs(context)}
      ...
      case 'dict_or_set':
        if(context.closed){
            ...
            if(token==='(') return new $CallArgCtx(new $CallCtx(context))
            if(token==='op'){
      ...
      case 'func_star_arg':
        ...
        if(token==',' && context.name===undefined){
      ...
      case 'list_comp':
        ...
        if(token==='if') return new $ExprCtx(context,'condition',true)
        $_SyntaxError(context,'token '+token+' after '+context)
      case 'node':
        switch(token) {
          case 'id':
      ...
      case 'raise':
        ...
        if(token=='from' && context.tree.length>0){
            return new $AbstractExprCtx(context,false)
        }
        if(token==='eol') return $transition(context.parent,token)
        $_SyntaxError(context,'token '+token+' after '+context)
